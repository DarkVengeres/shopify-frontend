<template>
    <form @submit.prevent="submitForm" id="promptForm">
        <textarea v-on:keyup.enter="submitForm" type="text" id="promptText" required v-model="prompt" placeholder="Enter a subject here..."></textarea>
...
export default {
    data() {
        return {
            prompt: "",
            isLoading: false,
            promptType: "Essay Outline"
        }
    },
    methods: {
        submitForm() {
            this.isLoading = true;
            this.$root.$refs.home.submit("Create an outline for an essay about :" + this.prompt, this.promptType);
            this.prompt = "";
        },
        loadingFinished() {
